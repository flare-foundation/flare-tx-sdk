            ...
            for (let wallet of wallets) {
                describe(wallet.getDescription(), async function () {
                    ...
                    it("transfer native", async () => {
                        ...
                        let recipientBalanceBefore = await network.getBalanceOnC(env.cAddress1)
                        await network.transferNative(wallet, env.cAddress1, testAmount)
                        let senderBalanceAfter = await network.getBalanceOnC(publicKey)
                        let recipientBalanceAfter = await network.getBalanceOnC(env.cAddress1)
                    ...
                    it("transfer wrapped", async () => {
                        ...
                        let recipientBalanceBefore = await network.getBalanceWrappedOnC(env.cAddress1)
                        await network.transferWrapped(wallet, env.cAddress1, testAmount)
                        let senderBalanceAfter = await network.getBalanceWrappedOnC(publicKey)
                        let recipientBalanceAfter = await network.getBalanceWrappedOnC(env.cAddress1)
